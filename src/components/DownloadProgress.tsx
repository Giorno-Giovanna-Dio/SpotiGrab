...
export default function DownloadProgress({
  ...
  const getStatusLabel = (statusKey: string) => {
    const key = `status.${statusKey}` as const;
    return t(key);
  };
      ...
      <div className="flex items-start gap-4">
        ...
        >
          {isComplete ? (
            ...
            <svg
              aria-hidden="true"
              viewBox="0 0 24 24"
              className={`h-5 w-5 ${!isFailed ? "animate-pulse" : ""}`}
              fill="none"
              stroke="currentColor"
              strokeWidth="1.8"
            >
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center justify-between gap-4">
            ...
            <span className="text-sm font-medium tabular-nums text-zinc-400">{percent}%</span>
          </div>
